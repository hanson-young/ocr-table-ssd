...
from .imJnet_ssd import SSD, GraphPath
from .predictor import Predictor
from .config import mobilenetv1_ssd_config as config
from .imJNetV3 import imJNetV3
...
def create_imJnet_ssd_lite(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False):
    mask_net = imJNetV3(num_classes=1, alpha=0.15, alpha_up=0.25)
    ...
    return SSD(num_classes, mask_net, base_net, source_layer_indexes,
               extras, classification_headers, regression_headers, is_test=is_test, config=config)


def create_imJnet_ssd_lite_predictor(net, candidate_size=200, nms_method=None, sigma=0.5, device=torch.device('cpu')):
