...
def rotate_map(mask, image, device):
    ...
    # draw_mask = (seg_mask * 255.)
    # re_mask = (rotate_mask * 255.)
    # cv2.imshow("om", cv2.applyColorMap(draw_mask.astype(np.uint8),cv2.COLORMAP_JET))
    # cv2.imshow("rm", cv2.applyColorMap(re_mask.astype(np.uint8),cv2.COLORMAP_JET))
    # cv2.waitKey(0)
