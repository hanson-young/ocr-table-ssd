...
class OcrDataset:
    ...
    def _get_annotation(self, image_id):
        ...
        for object in objects:
            ...
            boxes.append([point0, point1, point2, point3])
...
if __name__ == "__main__":
    ...
    for idx in range(len(dataset.ids)):
        ...
        cv2.polylines(total_gt_mask, gt_boxes,1,255,2)
        cv2.polylines(image, gt_boxes, 1, (255,255,255), 2)
        cv2.polylines(mask, gt_boxes, 1, 255, 7)
        cv2.fillPoly(mask, gt_boxes, 255)
        ...
        for jdx in range(0, len(contours)):
            ...
            area_box = []
            img_area = image[y: y + h, x: x + w, :]
            mask_area = total_gt_mask[y: y + h, x: x + w]
            area_list.append([img_area, mask_area])

    for area_ in area_list:
        ran_str = ''.join(random.sample(string.ascii_letters + string.digits, 20))
        ocr_cropped_img = os.path.join(ocr_cropped_root, 'segmentation', train_test, 'image', ran_str + ".jpg")
        ocr_cropped_mask = os.path.join(ocr_cropped_root, 'segmentation', train_test, 'mask', ran_str + ".jpg")
        # cv2.imwrite(ocr_cropped_img, area_[0])
        # cv2.imwrite(ocr_cropped_mask, area_[1])
        cv2.imshow("img",area_[0])
        cv2.imshow("mask", area_[1])
        cv2.waitKey(0)
